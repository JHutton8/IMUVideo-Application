(() => {
  ...
  // ============================================================
  // TEMP: same video on the tutorial START page (Hub) for all tutorials.
  // Later you can change this single path, or remove it and set the src in HTML.
  // ============================================================
  const HUB_VIDEO_SRC = "assets/videos/DEMO_2.mp4";
  ...
  window.MoveSyncPages[PAGE] = {
    mount() {
      ...
      const hubVideoEl = document.querySelector("#tutHubVideoEl");
      if (!hubHero || !grid || !runnerRoot) return;

      // Ensure the hub video always points to the configured file
      if (hubVideoEl && HUB_VIDEO_SRC) {
        hubVideoEl.src = HUB_VIDEO_SRC;
      }

      // Prevent browser from restoring scroll positions when navigating SPA pages
      // (some browsers restore scroll when DOM changes / history changes)
      try {
      ...
      // ------------------------------------------------------------
      // Scroll helpers: your app may scroll inside #mainContent (.home)
      // rather than window. We hard-reset both to guarantee top.
      // ------------------------------------------------------------
      function hardScrollTopNow() {
        // 1) If your app uses a scroll container (likely)
        const main = document.querySelector("#mainContent"); // <main class="home" id="mainContent">
        if (main && typeof main.scrollTo === "function") {
          main.scrollTo({ top: 0, behavior: "auto" });
          main.scrollTop = 0;
        }

        // 2) Fallback / also force the document scroller to top
        ...
        // 3) Also try window (harmless if not used)
        try {
          window.scrollTo({ top: 0, behavior: "auto" });
        } catch {}
      }

      function hardScrollTopStable() {
        // Win against layout shifts & any late scroll restores
        hardScrollTopNow();
        requestAnimationFrame(() => hardScrollTopNow());
        requestAnimationFrame(() => requestAnimationFrame(() => hardScrollTopNow()));
        setTimeout(() => hardScrollTopNow(), 0);
        setTimeout(() => hardScrollTopNow(), 50);
      }
      ...
      function showHub() {
        ...
        // Remove runner content completely (this also removes the nav from the DOM)
        ...
        // Ensure we are fully at the top when returning to hub
        hardScrollTopStable();

        // Optional: align hub hero to top
      ...
      function renderHub() {
        ...
        tutorials.forEach((tut) => {
          ...
          // Selecting a tutorial is the ONLY time we create runner + navigator
      ...
      function buildRunnerHTML(tut) {
        ...
        // Navigator is built dynamically from the selected tutorial’s steps:
        // Intro + N steps
        const navItems = [
          `
        ...
        return `
          <section class="mstut" aria-label="${tut.title} Tutorial">
            ...
            <!-- IMPORTANT: nav only exists in runner DOM -->
      ...
      function launchTutorial(id) {
        ...
        // Pause hub video when entering a tutorial
        if (hubVideoEl) {
          try {
            hubVideoEl.pause();
            hubVideoEl.currentTime = 0;
          } catch {}
        }

        // Hide hub and create runner (and only now: the nav)
        hubHero.hidden = true;
        runnerRoot.hidden = false;
        runnerRoot.innerHTML = buildRunnerHTML(tut);

        // ✅ CRITICAL: Force absolute top IMMEDIATELY after the runner is injected.
        // This guarantees Back to tutorials is visible without scrolling.
        hardScrollTopStable();
        ...
        function stepEl(n) {
          // 0 is the intro hero block (data-step="0") but it’s not in stepNodes
          if (n === 0) return runnerRoot.querySelector('.mstut-hero[data-step="0"]');
        ...
        function scrollToStep(n) {
          ...
          // Intro should align to top so the back button stays visible.
          const blockMode = (n === 0) ? "start" : "center";

          target.scrollIntoView({ behavior, block: blockMode });

          // Focus only for real steps (avoid stealing focus on intro)
          if (n !== 0) {
            setTimeout(() => {
        ...
        runnerRoot.querySelectorAll("[data-tut-top]").forEach((btn) => {
          btn.addEventListener("click", () => scrollToStep(0));
        });
        ...
        // We start at Intro state without moving the scroll position.
        currentStep = 0;
        setNavActive(0);
      }
